...
Hello, and welcome to the **introduction to data** tutorial!
...
This tutorial does not assume any previous R experience, but we will use R throughout the semester and this is meant to orientate you to using it.
...
- **dplyr**: Part of the tidyverse, dplyr provides functions for data manipulation like filtering, selecting, and summarizing data. 
- **ggplot2**: Also part of the tidyverse, ggplot2 is a powerful package for creating visualizations.
...
```{r load-packages, exercise=TRUE, exercise.cap="load-packages"}
library(dplyr)
library(ggplot2)
```
...
In this lesson we'll work with the High School and Beyond dataset, stored in a **data frame** called `hsb2`. You can read more about this dataset [here](http://openintrostat.github.io/openintro/reference/hsb2.html). Below is a 
...
```{r glimpse-hsb2, exercise=TRUE, exercise.cap="glimpse-hsb2"}
glimpse(hsb2)
```
...
```{r glimpse-email50, exercise=TRUE, exercise.cap="glimpse-email50"}
...
```{r glimpse-data, exercise=TRUE, exercise.cap="glimpse-data"}
...
```{r hsb2-count-schtyp, exercise=TRUE, exercise.cap="hsb2-count-schtyp"}
hsb2 |> 
  count(schtyp)
```
...
```{r hsb2-filter-schtyp-public, exercise=TRUE, exercise.cap="hsb2-filter-schtyp-public"}
hsb2_public <- hsb2 |>
  filter(schtyp == "public")
```
...
```{r hsb2-count-schtyp-again, exercise=TRUE, exercise.cap="hsb2-count-schtyp-again"}
hsb2_public |>
  count(schtyp)
```
...
```{r email50-filter-number-big, exercise=TRUE, exercise.cap="email50-filter-number-big"}
...
```{r hsb2-mean-read, exercise=TRUE, exercise.cap="hsb2-mean-read"}
...
```{r hsb2-mean-read-assign, exercise=TRUE, exercise.cap="hsb2-mean-read-assign"}
...
```{r hsb2-mean-read-assign-show, exercise=TRUE, exercise.cap="hsb2-mean-read-assign-show"}
(avg_read <- mean(hsb2$read))
```
...
```{r hsb2-mutate-read-cat, exercise=TRUE, exercise.cap="hsb2-mutate-read-cat"}
hsb2 <- hsb2 |>
...
```{r hsb2-mutate-num-char-cat, exercise=TRUE, exercise.cap="hsb2-mutate-num-char-cat"}
...
```{r email50-fortified, exercise=TRUE, exercise.cap="email50-fortified"}
...
```{r hsb2-science-math, exercise=TRUE, exercise.cap="hsb2-science-math"}
...
```{r hsb2-science-math-prog, exercise=TRUE, exercise.cap="hsb2-science-math-prog"}
...
```{r email50-plot, exercise=TRUE, exercise.cap="email50-plot"}
...
This tutorial was created on templates provided by OpenIntro:
