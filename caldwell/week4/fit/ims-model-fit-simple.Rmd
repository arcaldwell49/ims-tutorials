...
textbooks <- readRDS("data/textbooks.rds")
...
1. **Calculate and interpret measures of model accuracy**
...
3. **Identify and evaluate unusual observations**
   - Distinguish between leverage and influence
   - Calculate leverage scores using the `.hat` statistic
   - Measure influence using Cook's distance
   - Understand when an outlier affects your model

4. **Make informed decisions about outliers**
...
# Compute RMSE
sqrt(sum(residuals(___)^2) / df.residual(___))
```
...
```{r mc2}
question("Based on the RMSE you calculated, what does this tell you about the model's predictions?",
  answer("The model predictions are typically off by about [X] kg", correct = TRUE),
  answer("The model is perfect"),
  answer("The model explains [X]% of variance"),
  allow_retry = TRUE
)
```
...
```{r ex3-hint}
# Remember: var_y should be the variance of the response variable (wgt)
...
wgt_hgt_augment |>
  summarize(var_y = var(wgt),
            var_e = var(.resid)) |>
  mutate(R_squared = ___  - (var_e / var_y))
```
...
outlier <- Batting |>
  ...
  arrange(desc(HR)) |>
  dplyr::select(playerID, yearID, teamID, SB, HR) |>
  head(1) |>
  rename(stolen_base = SB,
         home_run = HR)

regulars_plus <- regulars |>
  dplyr::select(name, team, stolen_base, home_run) |>
  bind_rows(outlier)
...
1. **Is there a data entry error?** (Check the source)
2. **Does the observation represent a different population?** (Different scope)
3. **Is there a measurement error or equipment malfunction?** (Check the data collection)
4. **Does removing it fundamentally change your research question/interpretation?**


✅ Good reasons: Data errors, different population, measurement issues
❌ Bad reasons: "It doesn't fit my hypothesis", "It lowers my R²"
...
outlier <- Batting |>
  filter(SB > 60 & SB < 70 & HR > 20) |>
  dplyr::select(name = playerID, team = teamID, stolen_base = SB, home_run = HR) |>
...
2. Create a scatterplot with `obp` on the y-axis and `at_bat` on the x-axis for **both** the new and old datasets
...
1. **RMSE** tells you typical prediction error in original units
2. **R²** tells you the proportion of variance explained (0-1 scale)
3. **Leverage** depends only on x-values (distance from mean)
4. **Influence** combines leverage AND residual size
5. **Remove outliers only with strong justification** - document your reasoning!
