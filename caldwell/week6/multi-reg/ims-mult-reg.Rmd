---
title: "Week 6 (Part 2) - Multiple Regression"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
# Remove outliers: one auction had an unusually high price above $100
# This filtering makes the patterns in the data clearer for learning purposes
mariokart <- filter(mariokart, total_pr < 100)

# Required exercises for progress tracking
REQUIRED_EXERCISES <- c(
  "ex0-babies-glimpse",
  "ex1-babies-fit-model",
  "ex2-babies-data-space",
  "ex2-babies-facet",
  "ex3-babies-tiling-plane",
  "sandbox",
  "ex4-babies-model-space",
  "ex5-babies-plotly",
  "ex0-mariokart-glimpse",
  "ex1-mariokart",
  "ex2-mariokart",
  "ex3-mariokart",
  "ex6-babies-coef",
  "ex7-babies-model-space",
  "ex8-babies-model-hline",
  "ex9-babies-model-vline",
  "ex6-babies2",
  "ex4-mariokart",
  "ex10-babies-smoking",
  "ex10-babies-plotly-two",
  "ex5-mariokart",
  "ex6-mariokart",
  "ex11-babies",
  "ex12-babies",
...
Welcome! In this interactive tutorial, you'll learn how to build and interpret multiple regression models with several explanatory variables. Multiple regression is a powerful statistical tool that allows you to understand how multiple factors simultaneously influence an outcome of interest.

**Learning Objectives**

By the end of this tutorial, you will be able to:

1. **Build multiple regression models** with two or more numeric explanatory variables using R's `lm()` function
2. **Visualize multiple regression models** in 3D space using plotly
3. **Interpret regression coefficients** in the context of multiple predictors, including understanding the phrase "after controlling for"
4. **Construct parallel planes models** that incorporate both numeric and categorical explanatory variables
5. **Compare models** of increasing complexity and understand when to include additional variables
6. **Apply multiple regression** to real-world datasets (baby birthweights and eBay Mario Kart auctions)

**What You'll Work With**

We'll use two datasets throughout this tutorial:

- **`babies` dataset**: Data from the Child Health and Development Studies containing 1,236 observations of babies born in San Francisco (1960-1967). We'll model birthweight as a function of various maternal and pregnancy characteristics. [See documentation](http://openintrostat.github.io/openintro/reference/babies.html)

- **`mariokart` dataset**: Auction data from eBay for the Nintendo Wii game Mario Kart (October 2009) with 143 observations. You'll predict auction prices based on listing characteristics. [See documentation](https://www.openintro.org/data/index.php?data=mariokart)


As in the other lessons, the `dplyr` and `ggplot2` packages are already pre-loaded for data manipulation and visualization. When we use other packages, we will load them explicitly in the appropriate chunks.

## Building Models with Multiple Numeric Variables


Thus far, we've only considered regression models with one numeric explanatory variable. In this lesson, we will explore models that have at least two numeric explanatory variables.

### Why Multiple Regression?

Real-world phenomena are rarely influenced by just one factor. Consider:

- **Birthweight** depends on genetics, maternal health, prenatal care, gestational age, and more
- **Auction prices** reflect product condition, demand, timing, and seller reputation
- **Student performance** relates to study time, prior knowledge, teaching quality, and motivation

Multiple regression allows us to:
1. **Disentangle effects**: Isolate the impact of one variable while accounting for others
2. **Make better predictions**: Use multiple pieces of information simultaneously
3. **Test theories**: Examine whether specific factors matter after controlling for confounds
4. **Quantify relationships**: Put numbers on complex, multivariate relationships

The key phrase you'll hear throughout this tutorial is **"after controlling for"** or **"holding other variables constant."** This language reflects multiple regression's power to isolate effects.


### Adding a second numeric explanatory variable

Take a moment to identify the variables we'll be working with:
- **Response variable**: `bwt` (birthweight in ounces)
- **First predictor**: `gestation` (length of pregnancy in days)
- **Second predictor**: `age` (mother's age in years)


```{r ex0-babies-glimpse, exercise=TRUE, exercise.cap="ex0-babies-glimpse"}
glimpse(babies)
```
...
```{r ex1-babies-fit-model, echo=TRUE, exercise=TRUE, exercise.cap="ex1-babies-fit-model"}
...
# This syntax doesn't work - ggplot2 doesn't support a "z" aesthetic
...
```{r ex2-babies-data-space, echo=TRUE, exercise=TRUE, exercise.cap="ex2-babies-data-space", exercise.setup="ex1-babies-fit-model"}
...
Another way of handling this is to facet the data space by age groups. This approach is not ideal, because it requires us to categorize a numeric variable, which we generally want to avoid. But it does allow us to see the relationship between gestation and birthweight at different ages.

```{r ex2-babies-facet, echo=TRUE, exercise=TRUE, exercise.cap="ex2-babies-facet", exercise.setup="ex1-babies-fit-model"}
data_space <- ggplot(babies, aes(x = gestation, y = age)) + 
  geom_point()  +
  facet_grid(~cut(bwt, quantile(bwt, 
...
1. Firstly, we create a grid of values using the `data_grid()` function. With the `seq_range` function, we specify the spaces in the sequence of our variables. Using the parameter `by = 1` means that we want to produce all possible combinations. Both functions are part of the `modelr` package.
...
```{r ex3-babies-tiling-plane, echo=TRUE, exercise.setup="ex2-babies-data-space", exercise=TRUE, exercise.cap="ex3-babies-tiling-plane"}
library(modelr)
library(broom)
...
```{r ex4-babies-model-space, echo=TRUE, exercise=TRUE, exercise.cap="ex4-babies-model-space", exercise.setup="ex3-babies-tiling-plane"}
...
You've learned that when we include both numeric predictors and a categorical variable, our model creates parallel planes - one plane for each category. Let's visualize this relationship using the `plotly` package for 3D graphics.

**What to expect**: You'll create an interactive 3D scatterplot where:
- Each point represents one Mario Kart auction
- The x and y axes show the two numeric predictors
- The z axis shows the response variable (price)
- Color distinguishes between new and used items
- Two planes slice through the cloud of points, one for each condition
...
```{r ex5-babies-plotly, exercise.setup="ex3-babies-tiling-plane", exercise=TRUE, exercise.cap="ex5-babies-plotly", echo=TRUE, message=FALSE, exercise.lines=27}
library(modelr)
library(broom)
library(plotly)
...
## Your Turn: Building Your First Multiple Regression Model

Now it's your turn to apply what you've learned. In the following exercises, you'll work with the `mariokart` dataset to predict auction prices.

### Key Skills to Practice
- Identifying appropriate response and explanatory variables
- Fitting models with `lm()`
- Interpreting coefficients in context
- Using the "after controlling for" language correctly

Now you will fit a multiple linear regression model yourself. The dataset `mariokart` you are going to use is already loaded in your workspace. Have a first look with `glimpse()` at the data:

```{r ex0-mariokart-glimpse, exercise=TRUE, exercise.cap="ex0-mariokart-glimpse"}
___(___)
```
...
```{r ex1-mariokart, exercise=TRUE, exercise.cap="ex1-mariokart"}
...
```{r ex2-mariokart, exercise=TRUE, exercise.cap="ex2-mariokart"}
...
```{r ex3-mariokart, exercise=TRUE, exercise.cap="ex3-mariokart"}
# load plotly package
library(plotly) 
...
```{r ex6-babies-coef, exercise=TRUE, exercise.cap="ex6-babies-coef"}
...
```{r ex7-babies-model-space, exercise=TRUE, exercise.cap="ex7-babies-model-space", exercise.setup="ex4-babies-model-space"}
model_space
```
...
```{r ex8-babies-model-hline, exercise=TRUE, exercise.cap="ex8-babies-model-hline", exercise.setup="ex4-babies-model-space"}
model_space + 
...
```{r ex9-babies-model-vline, exercise=TRUE, exercise.cap="ex9-babies-model-vline", exercise.setup="ex8-babies-model-hline"}
model_space + 
...
```{r ex6-babies2, exercise=TRUE, exercise.cap="ex6-babies2"}
lm(___ ~ ___ + ___, data = ___)
```
...
### Quick Check: Understanding "Controlling For"

```{r mc_controlling}
question("What does it mean to interpret a coefficient 'after controlling for other variables'?",
  answer("We ignore the other variables in the model.", message = "No - we're acknowledging them, not ignoring them!"),
  answer("We assume the other variables don't change while we examine this one.", correct = TRUE),
  answer("We remove the other variables from the dataset.", message = "The variables stay in the model."),
  answer("We multiply our coefficient by the other coefficients.", message = "Coefficients are interpreted individually."),
  allow_retry = TRUE
)
```
...
```{r ex4-mariokart, exercise=TRUE, exercise.cap="ex4-mariokart"}
# Fit the model using duration and starting price
...
```{r ex10-babies-smoking, exercise=TRUE, exercise.cap="ex10-babies-smoking"}
...
```{r ex10-babies-plotly-two, exercise.setup="ex3-babies-tiling-plane", exercise=TRUE, exercise.cap="ex10-babies-plotly-two", exercise.lines=43}
library(broom)
library(modelr)
library(plotly)
...
We also needed to load the [broom](https://broom.tidymodels.org/) and [modelr](https://modelr.tidyverse.org/) and [plotly](https://plotly.com/r/) packages for this visualization.
...
```{r ex5-mariokart, exercise=TRUE, exercise.cap="ex5-mariokart"}
# load plotly
___(plotly)
...
```{r ex6-mariokart, exercise=TRUE, exercise.cap="ex6-mariokart"}
...
### The Blessing and Curse of More Variables

One of the most powerful aspects of multiple regression is that there's no theoretical limit to how many variables you can include. However, with great power comes great responsibility!

**Mathematical formulation**: Simply keep adding terms:

$$
\hat{bwt} = \hat{\beta}_0 + \hat{\beta}_1 \cdot gestation + \hat{\beta}_2 \cdot age + \hat{\beta}_3 \cdot smoke + \hat{\beta}_4 \cdot height + \hat{\beta}_5 \cdot weight + \hat{\beta}_6 \cdot parity
$$

**R syntax**: Just keep adding variables to your formula:

```{r ex11-babies, exercise=TRUE, exercise.cap="ex11-babies"}
lm(bwt ~ gestation + age + smoke + height + weight + parity, 
   data = babies)
```

Each of these variables might reasonably affect birthweight, so including them allows us to isolate the effect of each predictor while "controlling for" the others.

### The "Kitchen Sink" Approach

R provides a shortcut for including many variables at once using the dot operator (`.`), which means "all other variables in the dataset":

```{r ex12-babies, exercise=TRUE, exercise.cap="ex12-babies"}
lm(bwt ~ . - case, data = babies)
```

**What's happening here?**
- The `.` tells R to include all variables from the `babies` dataset
- The `- case` explicitly excludes the `case` variable (which is just an ID number and shouldn't be a predictor)

**Important caveat**: Just because you *can* include everything doesn't mean you *should*. Good modeling requires thoughtful variable selection based on theory, avoiding multicollinearity, and considering model complexity.

### Visualization Challenges in Higher Dimensions

Once we move beyond three dimensions, we can no longer create complete geometric visualizations. While mathematicians talk about "hyperplanes" in higher-dimensional spaces, we humans are stuck with our three-dimensional perception!

**Strategies for understanding higher-dimensional models:**
- Focus on coefficient interpretation rather than visualization
- Create 2D projections by fixing some variables at typical values
- Use the phrase "after controlling for" to acknowledge the other variables
- Examine pairwise relationships while keeping other variables in mind

### The Key to Interpretation

The fundamental interpretation remains unchanged: each coefficient represents the expected change in the response for a one-unit change in that predictor, **holding all other variables constant**.

For example, in our full babies model, the coefficient on `smoke` of -8.4 means: *The expected birthweight of babies born to mothers who smoke is 8.4 ounces lower than babies born to non-smoking mothers, after controlling for gestational length, maternal age, height, weight, and parity.*
...
```{r ex7-mariokart, exercise=TRUE, exercise.cap="ex7-mariokart"}
...
Congratulations on completing this tutorial on multiple regression! Let's review the key concepts:

### Core Concepts Covered

1. **Multiple regression syntax**: `lm(response ~ predictor1 + predictor2 + ..., data = dataset)`

2. **Interpreting coefficients**: Each coefficient represents the expected change in the response for a one-unit change in that predictor, **after controlling for** all other variables in the model

3. **3D visualization**: With two numeric predictors, data lives in 3D space and can be visualized using tools like plotly

4. **Parallel planes models**: When combining numeric and categorical predictors, the model creates parallel geometric surfaces (one for each category)

5. **Higher dimensions**: You can include many predictors, but visualization becomes impossible beyond three dimensions. Focus on interpretation instead.

### Key Phrases to Remember

- **"After controlling for..."** - acknowledges other variables in the model
- **"Holding other variables constant"** - same meaning as above
- **"After accounting for..."** - another equivalent phrase

### Common Pitfalls to Avoid

- ❌ Interpreting coefficients without mentioning other variables in large models
- ❌ Confusing correlation with causation (regression shows association, not causation)
- ❌ Including irrelevant variables (like ID numbers) as predictors
- ❌ Forgetting that categorical variable coefficients are relative to a reference level
