---
title: "Week 3 (Part 2): Interpreting regression models"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
1. **Causation vs. Correlation**: Regression shows association, not causation
2. **Switching Variables**: Keep response and explanatory variables straight
3. **Units Confusion**: Percentage vs. percentage points
4. **Scale Mismatches**: Comparing coefficients across different scales
...
question("Which of the following statements is **correct**?",
  answer("A person who is 170 cm tall is expected to weigh about 68 kg.", correct = TRUE),
...
# Show what happens with missing data
...
nrow(textbooks_missing)
length(fitted.values(books_mod_missing))
```
...
augment(books_mod) |>
  ...
  head() |>
  # for some emphasis...
  mutate(overpriced = ifelse(.resid > 30, "Significantly overpriced", "Normal"))
```
...
3. **Evaluate** how well the model generalizes
...
|----------|---------|---------|
| `lm()` | Fit the model | Model object |
| `coef()` | Extract coefficients | Numeric vector |
| `fitted.values()` | Get predicted values | Numeric vector |
| `residuals()` | Get residuals | Numeric vector |
| `predict()` | Predict new observations | Numeric vector |
| `augment()` | Tidy model output | Data frame |
