# =============================================================================
# Extract datasets for learnr tutorials
...
# (covers ims-correlation, ims-two-variables, ims-interpret-linear, ims-simple-linear)
...
for (ds_name in datasets_to_extract) {
  ...
  file_path <- file.path("caldwell",
                         "week3",
                         "simple_linear", "data", paste0(ds_name, ".rds"))
...
# Extract Galton dataset from mosaicData (used in ims-simple-linear)
cat("\nExtracting Galton dataset from mosaicData package...\n\n")
Galton <- mosaicData::Galton
file_path <- file.path("caldwell",
                       "week3",
                       "simple_linear", "data", paste0("Galton", ".rds"))
saveRDS(Galton, file_path)
cat(sprintf("%-20s: %s bytes (%d rows x %d cols)\n",
            "Galton.rds",
            format(file.size("data/Galton.rds"), big.mark = ","),
            nrow(Galton),
            ncol(Galton)))
