---
title: "Week 3 (Part 1): Simple linear regression"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
Galton <- readRDS("data/Galton.rds")
...
Notice how each line captures the data differently:
- **Through origin (slope 2.5)**: Too steep, misses the pattern
- **Through origin (slope 2.3)**: Better, but still constrained
- **With intercept (slope 1.3)**: Best captures the overall trend
...
Look at your scatterplot with the regression line. Consider:
- Does the relationship appear linear?
- Are there any unusual observations?
- How much variability is around the line?
...
```{r mc}
question("The slope coefficient in the regression model is:",
  answer("Y", message = "Not quite! Y represents the response variable."),
  answer("β₀", message = "Try again! β₀ represents the y-intercept."),
  answer("β₁", correct = TRUE, message = "Correct! β₁ represents the slope - how much Y changes for a one-unit change in X."),
  answer("ε", message = "Not quite! ε represents the error term or noise."),
  allow_retry = TRUE
)
```
...
```{r ex3-solution}
## The intercept formula comes from: y = b_0 + b_1 * x
...
✓ Visualize regression lines with `geom_smooth(method = "lm")`
✓ Understand that the least squares line minimizes $\sum e^2$
✓ Calculate slope as $r \cdot \frac{s_y}{s_x}$
✓ Interpret fitted values ($\hat{y}$) and residuals ($e$)
...
In the plot below, pay attention to:
- The **blue regression line** (what actually happens)
- The **dashed diagonal line** (if children were exactly as tall as parents)
- The **gap between them** (evidence of regression to the mean)
...
1. **Simple linear regression** models the relationship between two numeric variables
2. The **least squares criterion** finds the unique "best fit" line
3. **Regression to the mean** is a statistical phenomenon, not the same as regression modeling
4. In R, we visualize regression with `geom_smooth(method = "lm")`
