---
title: "Week 2 (Part 2): Correlation"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
# Load datasets from local RDS files (extracted from openintro)
run09 <- readRDS("data/run09.rds")
possum <- readRDS("data/possum.rds")
mammals <- readRDS("data/mammals.rds")
ncbirths <- readRDS("data/ncbirths.rds")
bdims <- readRDS("data/bdims.rds")
mlbbat10 <- readRDS("data/mlbbat10.rds")
county_complete <- readRDS("data/county_complete.rds")

# Prepare anscombe data (from base R datasets)
ansc_rows <- nrow(datasets::anscombe)

anscombe <- datasets::anscombe |>
  mutate(id = 1:ansc_rows) |>
  pivot_longer(cols = -id, names_to = "key", values_to = "val") |>
  mutate(
    variable = substr(key, 1, 1),
    set = substr(key, 2, 2)
  ) |>
  dplyr::select(-key) |>
...
# Create hypothetical survey data
set.seed(123)
survey <- data.frame(
  satisfaction = sample(1:5, 50, replace = TRUE),
  grade_code = sample(1:5, 50, replace = TRUE)  # 5=A, 4=B, 3=C, 2=D, 1=F
)
```
...
1. **Spearman's rho (ρ)** - Also called Spearman's rank correlation
2. **Kendall's tau (τ)** - Also called Kendall's rank correlation
...
**Pearson correlation** uses the actual values: 1, 2, 3, 4, 5 and 2, 4, 6, 8, 10.
...
study_data <- data.frame(
  hours = c(2, 3, 4, 5, 6, 25),  # 25 is an outlier
  score = c(65, 70, 75, 80, 85, 95)
)

ggplot(study_data, aes(x = hours, y = score)) +
  geom_point(size = 3) +
  geom_point(data = study_data[6,], color = "red", size = 5) +
  labs(title = "Study Hours vs Exam Score",
       subtitle = "Red point is an outlier")
```

```{r outlier-cors, echo=TRUE}
study_data |>
  summarize(
...
```{r monotonic-demo}
# Create data with a curved but monotonic relationship
set.seed(42)
n <- 100
monotonic_data <- data.frame(
  x = runif(n, 0, 10)
) |>
  mutate(y = log(x + 1) + rnorm(n, sd = 0.3))

ggplot(monotonic_data, aes(x = x, y = y)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue", linetype = "dashed") +
  labs(title = "Monotonic but Non-Linear Relationship",
       subtitle = "Blue line shows linear fit (poor match)")
```

```{r monotonic-cors, echo=TRUE}
monotonic_data |>
  summarize(
    Pearson = cor(x, y),
...
comparison <- data.frame(
  Feature = c("Type of data", "Measures", "Sensitive to outliers?", 
              "Handles non-linear relationships?", "Range", 
              "Computation speed", "Best for"),
  Pearson = c("Continuous", "Linear association", "Yes", "No", 
              "-1 to 1", "Fast", "Linear relationships with no outliers"),
  Spearman = c("Ordinal or continuous", "Monotonic association", "No", 
               "Yes (if monotonic)", "-1 to 1", "Moderate", 
               "Monotonic relationships, ordinal data"),
  Kendall = c("Ordinal or continuous", "Concordance", "No", 
              "Yes (if monotonic)", "-1 to 1", "Slower", 
              "Small samples, lots of tied ranks")
)

knitr::kable(comparison, caption = "Comparison of Correlation Methods")
```
...
Let's revisit the `mammals` data, where we saw a non-linear relationship between body weight and brain weight.

```{r ex-rank1, exercise=TRUE}
# Create scatterplot
...
Now compute all three correlation coefficients:

```{r ex-rank2, exercise=TRUE}
...
```{r mc-rank1}
question("Why is the Spearman correlation higher than the Pearson correlation for the mammals data?",
  answer("Spearman is always higher than Pearson", 
         message = "Not true! It depends on the data."),
  answer("The relationship is monotonic but not linear", 
         correct = TRUE, 
         message = "Exactly! Larger mammals tend to have larger brains consistently, but the relationship curves. Spearman captures this monotonic trend better."),
  answer("There are outliers in the data", 
         message = "While there might be outliers, the main issue is the non-linear pattern."),
  answer("Brain weight is measured on an ordinal scale", 
         message = "Brain weight is continuous, not ordinal."),
  allow_retry = TRUE
)
```
...
# Visualize with jitter since data is discrete
ggplot(survey, aes(x = satisfaction, y = grade_code)) +
  geom_jitter(width = 0.2, height = 0.2, alpha = 0.5) +
  scale_x_continuous(breaks = 1:5, 
                     labels = c("Very\nUnsatisfied", "Unsatisfied", 
                               "Neutral", "Satisfied", "Very\nSatisfied")) +
  scale_y_continuous(breaks = 1:5, labels = c("F", "D", "C", "B", "A")) +
  labs(title = "Course Satisfaction vs Expected Grade",
       x = "Satisfaction", y = "Expected Grade")
```

Compute the correlation:

```{r ex-ordinal, exercise=TRUE}
...
```{r mc-rank2}
question("Which correlation method would be LEAST appropriate for measuring the relationship between:\n(1) Temperature in Celsius and (2) Survey responses of 'Strongly Disagree' to 'Strongly Agree'?",
  answer("Pearson", correct = TRUE, 
         message = "Correct! Pearson requires continuous data. Survey responses are ordinal, making rank-based methods more appropriate."),
  answer("Spearman", 
         message = "Spearman works well with ordinal data!"),
  answer("Kendall", 
         message = "Kendall is designed for ordinal data!"),
  answer("All are equally appropriate", 
         message = "The type of data matters when choosing a correlation method."),
  allow_retry = TRUE
)
```

```{r mc-rank3}
question("If you have two continuous variables with a strong exponential relationship (always increasing but curved), which correlation coefficient will likely be HIGHEST?",
  answer("Pearson", 
         message = "Pearson measures linear relationships, so it may miss the strength of a curved relationship."),
  answer("Spearman", correct = TRUE, 
         message = "Correct! Spearman captures monotonic relationships regardless of whether they're linear or curved."),
  answer("Kendall", 
         message = "Kendall also captures monotonic relationships, but Spearman tends to be slightly higher for this type of data."),
  answer("They will all be the same", 
         message = "Different methods will give different values for non-linear data."),
  allow_retry = TRUE
)
```
...
✓ **Pearson** measures *linear* relationships (continuous data)
✓ **Spearman** measures *monotonic* relationships (ordinal or continuous data)
✓ **Kendall** also measures *monotonic* relationships with better properties for small samples
✓ Rank-based methods (Spearman, Kendall) are robust to outliers
✓ For ordinal data (like survey responses), always use rank-based methods
✓ When in doubt, compute all three and understand why they differ!

**In R:**

- `cor(x, y)` → Pearson (default)
...
These four datasets are famous in statistics because they demonstrate why **visualization must always accompany numerical summaries**. Each has:
- Same number of points (n = 11)
- Same mean of x and y
- Same standard deviation of x and y  
- Same correlation (r = 0.82)
...
```{r ex6, exercise = TRUE, exercise.cap = "ex6"}
# Run this and look at the plot
ggplot(data = mammals, aes(x = body_wt, y = brain_wt)) +
  geom_point() + 
  scale_x_log10() + 
...
1. X causes Y
2. Y causes X  
3. A third variable (confounder) causes both X and Y
4. Pure coincidence
...
```{r ex7, exercise = TRUE, exercise.cap = "ex7", fig.height=5, fig.width=7, exercise.setup="prepare-noise"}
...
```{r ex8, exercise = TRUE, exercise.cap = "ex8", exercise.setup="prepare-noise"}
...
```{r ex9, exercise = TRUE, exercise.cap = "ex9", exercise.setup="prepare-noise-summary"}
...
## Adding Correlation Statistics to Plots

### The stat_cor() function

While we've learned how to calculate correlation coefficients using `cor()` and `cor.test()`, it's often useful to display the correlation directly on your scatterplot. The `ggpubr` package provides a convenient function called `stat_cor()` that adds correlation statistics as text to your plots.

First, let's load the package:

```{r load-ggpubr, echo = TRUE}
library(ggpubr)
```

### Basic usage

The `stat_cor()` function works as an additional layer in your `ggplot()` call, similar to how we add `geom_point()` or `geom_smooth()`. Here's a basic example using the `possum` dataset:

```{r stat-cor-basic, exercise = TRUE}
...
By default, `stat_cor()` displays the correlation coefficient (R) and the p-value in the upper left corner of the plot.
...
```{r stat-cor-method, exercise = TRUE}
...
```{r stat-cor-position, exercise = TRUE}
# Place in bottom right corner
ggplot(data = possum, aes(x = tail_l, y = total_l)) +
  geom_point() +
  stat_cor(label.x = 42, label.y = 77)
```

The `label.x` and `label.y` arguments control where the text appears. You'll need to adjust these based on the range of your data.

**Displaying only certain statistics:**

```{r stat-cor-label, exercise = TRUE}
...
Let's apply this to one of our earlier examples with the `bdims` dataset:

```{r stat-cor-bdims, exercise = TRUE}
...
Use the `mlbbat10` dataset to create a scatterplot of slugging percentage (`slg`) versus on-base percentage (`obp`) for players with at least 200 at-bats. Include:
- Points
- A regression line
- The correlation coefficient and p-value using `stat_cor()`
- Appropriate axis labels

```{r ex-stat-cor, exercise = TRUE}
# Filter the data
...
- Creating quick exploratory plots where you want to see the correlation at a glance
- Making presentations where you want correlation information visible on the plot
- Comparing multiple relationships side-by-side (especially with faceted plots)

However, for formal statistical reports, you should still report the full results from `cor.test()`, which provides confidence intervals and additional statistical information.

### Key takeaways

✓ `stat_cor()` adds correlation statistics directly to your ggplot
✓ You can customize the method (Pearson, Spearman, Kendall)
✓ Position and displayed statistics are customizable
✓ It's a convenience function for visualization, not a replacement for formal testing
